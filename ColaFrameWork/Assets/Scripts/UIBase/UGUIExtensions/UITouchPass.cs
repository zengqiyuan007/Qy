...
namespace UnityEngine.UI.Extensions
{
    public class UITouchPass : MonoBehaviour, IPointerClickHandler,
        ...
        , ISubmitHandler, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler,
        IScrollHandler
    {
        ...
        public virtual void OnPointerDown(PointerEventData eventData)
        {
            ...
            if (Input.GetButtonDown("Submit"))
                ExecuteEvents.Execute(eventData.pointerCurrentRaycast.gameObject, eventData,
        ...
        GameObject PassEvent<T>(BaseEventData data, ExecuteEvents.EventFunction<T> function)
            where T : IEventSystemHandler
        {
            ...
            foreach (var item in result)
            {
                ...
                if (go != null && go != pointerGo)
                {
                    ...
                    if (excuteGo)
                    {
                        if (null != excuteGo.GetComponent<UITouchPass>())
                            return null;
                    ...
                    {
                        var com = go.GetComponent<UnityEngine.UI.Graphic>();
                        if (null != com)
                        {
